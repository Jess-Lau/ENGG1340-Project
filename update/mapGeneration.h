...
void printMap(std::string roomMap[][7], const int roomLength, const int roomWidth) {
...
void printStat(std::string currentRoom, std::vector<std::string> inventory) {
    std::cout    << "You are in the " << currentRoom << "\n"
            << "Inventory: ";
...
bool isItemNeargy(std::string currentRoom, std::map<std::string, Rooms> rooms) {
...
bool isInInventory(std::vector<std::string> inventory, std::string item) {
    for (int i = 0; i < inventory.size(); i++) {
        if (inventory[i] == item) {
            return true;
        }
